...
void player_attack(Liste_t*player, t_pkm *wild_pkm){
  FILE * file;
  char choix[50];
  char dir[50] = "atk/";
  char nom[50];
...
void combat(Liste_t *player, t_pkm *wild_pkm){
  int tour = 0;
  if (player->pkm.sp > wild_pkm->sp)
    printf("%s commence\n", player->pkm.pkm_name);
  else{
    printf("%s commence\n", wild_pkm->pkm_name);
    tour++;
  ...
  while(player->pkm.hp > 0 && wild_pkm->hp >0){
    affiche_Stats(player, wild_pkm);
    if (tour%2==0){
      int choice = player_choice();
      tour++;
      if (choice == 1){
        player_attack(player, wild_pkm);
      }
      else if (choice == 0){
